...
class AlphaPay_API{
    ...
    public function generate_alphapay_order($order,$channel,$api_uri){

		$currency =method_exists($order, 'get_currency') ?$order->get_currency():$order->currency;
		
		if($currency != 'CNY' && $currency != 'CAD'){
			throw new Exception('Current Payment Method Only Accept Currency: CNY & CAD');
		}
        ...
        if(!$resArr){
            throw new Exception('This request has been rejected by the Alpha Pay service!');
        }
    ...
    public function alphapay_refund($amount,$ooid,$refund_id){
		...
		$url ="https://pay.alphapay.ca/api/v1.0/gateway/partners/$partner_code/orders/$ooid/refunds/$refund_id";
		$url.="?time=$time&nonce_str=$nonce_str&sign=$sign";
    ...
    public function query_order_status($alphapay_order_id){
		...
		$url ="https://pay.alphapay.ca/api/v1.0/gateway/partners/$partner_code/orders/$alphapay_order_id";
		$url.="?time=$time&nonce_str=$nonce_str&sign=$sign";
	...
	public function wc_alphapay_notify(){

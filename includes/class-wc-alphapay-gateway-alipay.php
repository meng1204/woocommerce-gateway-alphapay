...
class WC_AlphaPay_Alipay extends WC_Payment_Gateway{	
	...
	public function __construct(){
		...
		$this->method_title='Alpha Pay - Alipay';
	...
	public function process_payment($order_id){
		...
		try {
			if($this->is_app_client()){
				$result = AlphaPay_API::generate_alphapay_order($order,'Alipay',"https://pay.alphapay.ca//api/v1.0/h5_payment/partners/%s/orders/%s");
			}else{
				if('1'==$this->get_option('qrcode_redirect')){
					$result = AlphaPay_API::generate_alphapay_order($order,'Alipay',"https://pay.alphapay.ca/api/v1.0/alipay/partners/%s/orders/%s");
					
				}else{
					$result = AlphaPay_API::generate_alphapay_order($order,'Alipay',"https://pay.alphapay.ca/api/v1.0/gateway/partners/%s/orders/%s");
				}   
			}
